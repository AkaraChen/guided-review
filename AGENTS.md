...
`src/render.rs`. HTML lives in `templates/`; `examples/review.json` is the fixed
development payload. Integration tests belong in `tests/`. Generated files and
build artifacts stay under `target/`.
...
exists. Until one emerges, use concise Conventional Commit subjects such as
